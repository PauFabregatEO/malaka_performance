...
@cp.fuse()
def jacobi_update(u, north, south, east, west, mask):
    return cp.where(mask, 0.25 * (north + south + east + west), u)
...
def jacobi_gpu_cupy(u, interior_mask, max_iter=20000):
    ...
    for _ in range(max_iter):
        u_new_device[:, 1:-1, 1:-1] = jacobi_update(
            u_device[:, 1:-1, 1:-1],
            u_device[:, 2:, 1:-1],    # north
            u_device[:, :-2, 1:-1],   # south
            u_device[:, 1:-1, 2:],    # east
            u_device[:, 1:-1, :-2],   # west
            mask_device
        )
